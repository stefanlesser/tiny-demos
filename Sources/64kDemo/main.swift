...
func loadEmbeddedResource(segment: String, section: String) -> Data {
    ...
    guard
        ...
        // Fallback to reading from resources folder when not embedded (e.g., swift run)
        let bundle = Bundle.main
        if let url = bundle.url(forResource: "Shaders", withExtension: "metallib") {
            return try! Data(contentsOf: url)
        }
        
        // If that also fails, try to find it in the current directory or relative path
        let currentDir = FileManager.default.currentDirectoryPath
        let resourcePath = "\(currentDir)/Shaders.metallib"
        if FileManager.default.fileExists(atPath: resourcePath) {
            return try! Data(contentsOf: URL(fileURLWithPath: resourcePath))
        }
        
        fatalError("Embedded Metal library section not found and fallbacks failed")
    }
